...
//		 voucherHeader.getVouchermis().setDepartmentcode(getDefaultDepartmentValueForPayment());
	...
	public void setOneFunctionCenterValue() {
		...
		if (appConfigValues == null || appConfigValues.isEmpty())
			...
			voucherHeader.setIsRestrictedtoOneFunctionCenter(
					appConfigValues.get(0).getValue().equalsIgnoreCase("yes") ? true : false);
	}
	...
	protected void loadFundSource() {
		if (headerFields.contains("fundsource") && null != voucherHeader.getVouchermis().getSubschemeid()) {
			final List<Fundsource> fundSourceList = financingSourceService
					.getFinancialSourceBasedOnSubScheme(voucherHeader.getVouchermis().getSubschemeid().getId());
	...
	protected void validateFields() {
		...
		checkMandatoryField("functionary", voucherHeader.getVouchermis().getFunctionary(),
				...
				if (voucherDetail.getFunctionIdDetail() != null)
					if (voucherHeader.getIsRestrictedtoOneFunctionCenter())
						detailMap.put(VoucherConstant.FUNCTIONCODE,
								voucherHeader.getVouchermis().getFunction().getCode());
	...
	protected boolean validateData(final List<VoucherDetails> billDetailslist,
			final List<VoucherDetails> subLedgerList) {
		...
		for (final VoucherDetails voucherDetails : billDetailslist) {
			index = index + 1;
			totalDrAmt = totalDrAmt.add(voucherDetails.getDebitAmountDetail() == null ? BigDecimal.ZERO
					: voucherDetails.getDebitAmountDetail());
			totalCrAmt = totalCrAmt.add(voucherDetails.getCreditAmountDetail() == null ? BigDecimal.ZERO
					: voucherDetails.getCreditAmountDetail());
			...
			if (voucherDetails.getDebitAmountDetail().compareTo(BigDecimal.ZERO) == 0
				...
				isValFailed = true;
					...
					|| voucherDetails.getCreditAmountDetail().compareTo(BigDecimal.ZERO) > 0)
					&& voucherDetails.getGlcodeDetail().trim().length() == 0
					|| voucherDetails.getGlcodeIdDetail() == null) {
				addActionError(
						getText("journalvoucher.accdetail.accmissing", new String[] { Integer.toString(index) }));
				isValFailed = true;
			} else if (voucherDetails.getFunctionDetail() != null
					&& !voucherDetails.getFunctionDetail().equalsIgnoreCase("")
				...
				functionId = voucherDetails.getFunctionIdDetail() != null
						? voucherDetails.getFunctionIdDetail().toString() : "0";
				...
				if (null == existingFuncs) {
					...
					addActionError(
							getText("jv.multiplecodes.funcMissing", new String[] { voucherDetails.getGlcodeDetail() }));
					isValFailed = true;
					break;
	...
	protected boolean validateSubledgerDetails(final List<VoucherDetails> billDetailslist,
		...
		final List<Map<String, Object>> subLegAccMap = new ArrayList<>(); // this
																			// list
																			// will
																			// contain
																			// the
		...
		for (final VoucherDetails voucherDetails : billDetailslist) {
			...
			if (null != chartOfAccountDetail) {
				...
				if (repeatedglCodes.contains(voucherDetails.getGlcodeIdDetail().toString()))
					 ...
					 * voucherDetails.getGlcodeIdDetail()+'-'+voucherHeader.
					 * getFunctionId()); }else{
					 */
					accountDetailMap.put("glcodeId-funcId", voucherDetails.getGlcodeIdDetail().toString() + '-'
							+ voucherDetails.getFunctionIdDetail().toString());
				// }
				else
		...
		for (final VoucherDetails voucherDetails : subLedgerlist) {
			if (voucherDetails.getGlcode() == null) {
				addActionError(
						getText("journalvoucher.acccode.missing", new String[] { voucherDetails.getSubledgerCode() }));
				return true;
			}
			if (voucherDetails.getGlcode().getId() != 0) {
				final String function = repeatedglCodes.contains(voucherDetails.getGlcode().getId().toString())
						? voucherDetails.getFunctionDetail() : "0";
				// above is used to check if this account code is used multiple
				// times in the account grid or not, if it used
				// multiple times
				// then take the function into consideration while calculating
				// the total sl amount , else igone the function by
				// paasing function value=0
				if (null != subledAmtmap.get(voucherDetails.getGlcode().getId().toString() + '-' + function)) {
					final BigDecimal debitTotalAmount = subledAmtmap
							.get(voucherDetails.getGlcode().getId().toString() + '-' + function)
			...
			for (final Map<String, Object> map : subLegAccMap) {
				...
				if (null == subledAmtmap.get(glcodeIdAndFuncId)) {
					...
					return true;
						...
						.compareTo(new BigDecimal(map.get("amount").toString())) != 0) {
					final String functionId = glcodeIdAndFuncId.split("-")[1];
					if (functionId.equalsIgnoreCase("0"))
						addActionError(getText("journalvoucher.subledger.amtnotmatchinng",
								new String[] { map.get("glcode").toString() }));
					else {
						final CFunction function = (CFunction) persistenceService.find("from CFunction where id=?",
								Long.valueOf(functionId));
						addActionError(getText("journalvoucher.subledger.amtnotmatchinngFunc",
								new String[] { map.get("glcode").toString(), function.getName() }));
					}
					return true;
				}
	...
	public void saveReverse(final String voucherName, final String type) {
		...
		try {
			...
			throw new ValidationException(
					Arrays.asList(new ValidationError("invalid.date.format", "invalid.date.format")));
		}
	...
	public void setFinancingSourceService(final FinancingSourceService financingSourceService) {
